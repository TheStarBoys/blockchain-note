...
Q：在一个函数里面声明变量 `uint256 a;` 不加关键字、加 memory 和加 storage 有什么区别？

A：`uint256 a;` 相当于会在栈里分配，但EVM栈是有大小限制的，可能不是最佳做法。 `uint256 memory a; ` 的话会在堆中分配，`uint256 storage a;` ？
